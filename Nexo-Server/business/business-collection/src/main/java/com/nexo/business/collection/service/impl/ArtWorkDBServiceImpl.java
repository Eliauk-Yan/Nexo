...
public class ArtWorkDBServiceImpl extends BaseArtWorkServiceImpl {
    ...
    public NFTDetailVO getNFTDetailById(Long id) {
        ...
        if (now.isAfter(detail.getSaleTime())) {
            if (inventory > 0) {
                detail.setProductState(ProductState.SELLING);
            } else {
                detail.setProductState(ProductState.SOLD_OUT);
            }
        } else {
            if (Duration.between(now, detail.getSaleTime()).toMinutes() > DEFAULT_COMING_SOON_TIME) {
                detail.setProductState(ProductState.WAIT_FOR_SALE);
            } else {
                detail.setProductState(ProductState.COMING_SOON);
            }
        }
        // 8. 返回详细信息
        return detail;
